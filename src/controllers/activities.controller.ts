...
export class ActivitiesController {
    ...
    public createActivity = async (req: Request, res: Response) => {
        try {
            ...
            this.addActivityToPlanning(req.body.campus, newActivity._id);
    ...
    public addActivityToPlanning = async (campus: string, activityId: mongoose.Types.ObjectId) => {
    ...
    public updateActivity = async (req: Request, res: Response) => {
        try {
            ...
            return (!actividadActualizada)
                ? res.status(404).json({ error: 'Actividad no encontrada o no se pudo realizar la actualizacion' })
    ...
    public uploadPoster = async (req: Request, res: Response) => {
        try {
            const activity = await ActivityModel.findById(req.params.id);
            if (!activity) {
                return res.status(404).json({ error: 'Actividad no encontrada' });
            ...
            if (!poster) {
                return res.status(400).json({ error: 'No se ha proporcionado un póster' });
            }
            ...
            return res.status(500).json({ error: 'Error al subir el póster' });
        }
    }
}
